...
def home():
    html_content = """
        ...
        <p>Be sure to visit http://127.0.0.1:5000/api/jokes/3 for 3 funny jokes!</p>
    """
    return html_content
...
def get_jokes(n):
    if n < 1:
        ...
        return jsonify({"error": f"Number of jokes must not exceed {len(jokes)}"}), 400
    else:
        return jsonify(random.sample(jokes, n))
